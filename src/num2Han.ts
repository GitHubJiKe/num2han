...
type TMode = "简" | "繁";
class Number2Han {
    private mode: TMode = "简";

    private get num_map() {
        return this.mode === "简" ? num_map : num_map_fan;
    }

    private get unit_map() {
        return this.mode === "简" ? unit_map : unit_map_fan;
    }
    ...
    convert(num: number | string) {
        ...
        const decimal = () => {
            if (decimalStr) {
                const val = Array.from(decimalStr).reduce(
                    (a, c) => `${a}${this.num_map[c as TNumKey]}`,
                    ""
                );
                return `点${val}`;
            }
        ...
        if (len < 2) {
            return `${prefix}${this.num_map[numStr as TNumKey]}${decimal()}`;
        }
        if (len === 2) {
            const val =
                numStr[0] === "1"
                    ? this.unit_map["1"]
                    : `${this.num_map[numStr[0] as TNumKey]}${
                          this.unit_map["1"]
                      }`;
            return `${prefix}${val}${
                this.num_map[numStr[1] as TNumKey]
            }${decimal()}`;
        }
        const val = Array.from(numStr)
            ...
            .reduce((a, c, i) => {
                const idx = Number(i);
                const value = this.num_map[c as TNumKey];
                let unitIdx = idx % 4 === 0 && idx > 0 ? 4 : idx % 4;
                if (idx > 7) {
                    unitIdx = idx === 8 ? idx : idx % 8;
                }
                const unit = this.unit_map[unitIdx.toString() as TUnitKey];
                return `${value}${unit || ""}${a}`;
            }, decimal());
        return `${prefix}${val}`;
    }
