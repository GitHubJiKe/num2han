...
describe("utils", () => {
  ...
  test("千万位数", () => {
    expect(num2Han.convert(87651234)).toBe("八千七百六十五万一千二百三十四");
    expect(num2Han.convert(99999999)).toBe("九千九百九十九万九千九百九十九");
  });
  test("亿位数", () => {
  ...
  test("设置新的配置", () => {
    expect(num2Han.convert(1.23, "繁")).toBe("壹点贰叁");
    expect(num2Han.convert(187651234, "繁")).toBe(
      "壹億捌仟柒佰陆拾伍萬壹仟贰佰叁拾肆"
    );
  });
});
